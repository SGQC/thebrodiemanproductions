...
namespace CrittersAchievement
{
    public class CrittersAchievement : HBPlugin
    {
        ...
        { Logging.Write(Colors.SeaGreen, Name + ": " + format, args); }
        ...
        public void updateList(bool slog_)
        {
            ...
            #region AchievementID 6350
            if (!Completed(6350))
            {
                int done = 0;
                int AchievementMaxCriteria = Lua.GetReturnVal<int>("local value = GetAchievementNumCriteria(6350); return value", 0);

                for (int i = 1; i <= AchievementMaxCriteria; ++i)
                {
                    string value = RunLuaString(6350, i, "quantityString");
                    string name = RunLuaString(6350, i, "criteriaString");

                    if (value.ToInt32() == 0) { loveAchievement.Add(name); ++done; }
                }
                done = (AchievementMaxCriteria - done);
                if (slog_) { slog("[{0}] ({1}/{2})  uncomplete! Search and love..", AchievementName(6350), done, AchievementMaxCriteria); }
            }
            else { if (slog_) { slog("[" + AchievementName(6350) + "] is complete!"); } ++_complete; }
            #endregion
