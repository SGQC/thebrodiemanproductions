...
namespace Blastranaar
{
    public class Blastranaar : CustomForcedBehavior
    {
        ...
        public Composite SerpentFlyTo
        {
			...
			{
				TreeRoot.StatusText = "Moving to Subjugated Serpent";
				WoWMovement.ClickToMove(Serpent[0].Location);
				Serpent[0].Target();
		    		Serpent[0].Face();
                    		Thread.Sleep(2000);
				if (Serpent[0].Location.Distance(Me.Location) < 25)
        ...
        {
            return _root ?? (_root = new Decorator(ret => !_isBehaviorDone, new PrioritySelector(DoneYet, SerpentFlyTo, new ActionAlwaysSucceed())));
        }
    }
}
